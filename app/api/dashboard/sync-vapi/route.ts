...
export async function POST(request: NextRequest) {
  ...
  const credRows = await sql`
    SELECT vapi_api_key FROM tenant_credentials WHERE tenant_id = ${tenant.id} LIMIT 1`
  const cred = (credRows as { vapi_api_key: string | null }[])[0]
  const vapiApiKey = cred?.vapi_api_key?.trim()
  if (!vapiApiKey) {
    return NextResponse.json(
      { error: 'Add your Vapi API key in Settings first' },
      { status: 400 }
    )
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: `Vapi API error: ${res.status}` },
        { status: 502 }
      )
    }
  ...
  for (const v of vapiCalls) {
    ...
    const existing = await sql`
      SELECT id FROM conversations WHERE tenant_id = ${tenant.id} AND external_id = ${externalId} LIMIT 1`
    ...
    if (twilioSid) {
      const callRows = await sql`
        SELECT id FROM calls WHERE tenant_id = ${tenant.id} AND call_sid = ${twilioSid} LIMIT 1`
      const c = (callRows as { id: string }[])[0]
      if (c) callId = c.id
      else {
        const ins = await sql`
          INSERT INTO calls (tenant_id, call_sid, duration_sec, status, created_at, updated_at)
          VALUES (${tenant.id}, ${twilioSid}, ${v.duration ?? 0}, 'completed', ${v.createdAt ? new Date(v.createdAt).toISOString() : new Date().toISOString()}, now())
    ...
    try {
      await sql`
        ...
        VALUES (${tenant.id}, ${callId}, ${externalId}, ${intent}, ${outcome}, ${durationSec}, 0, ${JSON.stringify(v)}::jsonb, ${v.createdAt ? new Date(v.createdAt).toISOString() : new Date().toISOString()}, now())`
      imported += 1
    } catch {
